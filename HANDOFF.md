...
- 앱별 무료 구독 등록 안내와 문구·문서 개선은 BATON `ea23b91b`, `codex/free-calendar-registration`에 있다.
...
| 기준 | 범위·환경 | 결과와 제한 |
| --- | --- | --- |
| CAL `d628158` + 이번 문서 변경 | 2026-09-08, 수정 문서의 상대 링크·변경한 제목 참조 확인, `git diff --check`, Java 25.0.3의 `./gradlew --no-daemon verifyContractsZip` | 모두 성공. ZIP 작업 2개 새 실행, ZIP 안의 최신 문서 2개가 원본과 일치. 로그: `/private/tmp/baton-cal-wording-followup-20260908.log`. 코드·테스트·스키마·골든 파일 변경이 없어 앱 테스트는 반복하지 않음. 릴리스·배포 명령은 실행하지 않음 |
| BATON `ea23b91b` | 2026-09-08, Node 25.4.0·npm 11.7.0. `npm run typecheck`, `npm run build`, 구독·목록 E2E의 오류 시나리오 4개 | 타입·빌드 성공, PC·390px 모바일·WebKit 12개 새 실행 성공, 실패·제외 0개. 발급 응답 유실·잘못된 주소·목록 조회 실패·다음 페이지 실패 확인. 서버·외부 계정 실제 등록·운영 검증 제외. 작업 경로의 `output/verification/latest.md`에 명령과 로그 위치 기록 |
