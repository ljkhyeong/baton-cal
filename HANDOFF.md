...
- 웹훅 스모크는 모의 API의 `429`·`503` 뒤 전달 재개와 오류 로그의 URL 비노출을 검사한다.
  실패한 시도와 실제 수신 기록을 분리한다. 기본 재시도는 Alertmanager가 맡으며 제품 코드는 추가하지 않았다.
...
- 최신 기준은 `b0369f0`다. 웹훅 스모크·모의 수신기와 문서만 변경했다.
