...
- V4는 기존 구독 행에 호환용 초기 세대를 저장한 뒤 열 기본값을 제거한다. 이후 저장할 세대 값은
  애플리케이션이 지정한다. pre-V4는 구독 세대를 저장·검사하지 않으므로 모든 구버전을 중지한 상태에서
  적용하며 V4 이후 pre-V4 롤백과 구·신 버전 공존을 허용하지 않는다.
- V5는 더 이상 읽지 않는 `season_feed_projection.item_count`를 제거한다. 이 열을 참조하는 pre-V5
  인스턴스를 모두 중지한 유지보수 배포로 적용한다. 이후 pre-V5 롤백·공존은 금지하며,
  문제가 생기면 신버전의 오류를 수정해 재배포한다.
...
- V7는 시즌별 표시 이름·원본 개정 번호·채택 시각을 `season_calendar_metadata`에 보관한다.
  ...
  과거 이름 이력·이벤트 수신 기록·별도 해시는 필요하지 않아 두지 않는다. 채택 시각은 이름만 있는 첫 피드의
...
- V8은 복구 실행별 시즌 상태 검증과 전체 완료를 별도 테이블에 보관한다. 목록 원문은 복제하지 않고
  ...
  같은 복구 ID의 요청은 트랜잭션 advisory lock으로 직렬화한다. 최초 완료는 검증 후 일반 INSERT하고,
  재시도는 저장된 완료 내용과 비교해 최초 완료 시각을 반환한다. 저장 직후 재조회는 하지 않는다.
  시즌 잠금은 현재 상태를 대조하는 완료 전 검증에만 사용한다. 완료 후 매니페스트 재시도는
  복구 ID 잠금 안에서 저장된 기록만 비교하므로 시즌 잠금을 잡지 않는다.
  시즌 검증값의 저장·조회·비교에는 `RecoverySeasonState`를 재사용한다. 별도의 행 모델로 복사하지
  않으며 조회에는 비교할 필드만 포함한다. 검증 시각은 기존대로 DB에 저장한다.
...
- `UTC_INSTANT`·`ZONED_LOCAL` 구간은 `DTSTART`와 `DTEND`, `UTC_POINT`·`ZONED_LOCAL_POINT`
  단일 시점은 `DTSTART`만 투영한다. `ALL_DAY`는 `VALUE=DATE`인 시작·종료 날짜를 사용하며
  시작 날짜는 포함하고 종료 날짜는 제외한다. [RFC 5545 3.6.1절](https://www.rfc-editor.org/rfc/rfc5545.html#section-3.6.1)을 따른다.
...
계약 버전은 `contracts/VERSION`에서 관리한다. Gradle 표준 `Zip` 작업
...
GitHub Actions의 `upload-artifact`는 검토용 ZIP의 보존 기간을 `retention-days: 90`으로 요청한다.
실제 만료는 저장소·조직 정책을 따른다. 공식 계약은 다음 순서로 게시·채택한다.

1. 릴리스 대상 변경을 검토하고 풀 리퀘스트의 CI를 통과시켜 `main`에 반영한다.
2. `main`의 깨끗한 체크아웃에서 검증한 ZIP을 초안 릴리스에 첨부한다. 저장소의
   `Enable release immutability`를 켜고 사전 릴리스로 게시한다.
3. BATON은 `gh release verify`, `gh release verify-asset`으로 릴리스 증명과 자산을 확인한 뒤
   해당 버전을 고정하고 생산자 계약을 검증한다.
4. 계약 의미를 바꿀 필요가 없으면 `contracts/VERSION`을 안정 버전으로 올려 새 ZIP과 태그를 만든다.
   의미 변경이 필요하면 게시한 사전 릴리스를 교체하지 않고 다음 사전 릴리스를 만든다.

CI 산출물만으로 생산자 연동이 완료됐다고 판단하지 않는다.
...
- `prod`는 `management.endpoint.health.probes.add-additional-paths=true`로 API 포트에도 `/livez`와
...
- 실제 PostgreSQL 논리 백업·복원 스모크는 백업 복원 전 구독 세대 교체, 대표 원본 재전달, 자격 증명
  재발급 순서를 컨테이너 환경에서 검증한다.
...
- V4는 구독 세대를 검사하지 않는 구버전과, V5는 제거된 투영 열을 참조하는 구버전과 함께 실행할 수 없다.
  V6의 새 시간 형태도 pre-V6가 해석하지 못하므로 배포 전 구버전을 중지하고, 오류는 신버전에서 수정해야 한다.
...
- CAL은 V8 매니페스트로 BATON이 제시한 최신 상태와 저장 상태의 일치를 확인한다. BATON 원본의
  전체 목록 작성과 복구 모드 설정·완료 응답 확인 뒤의 해제는 BATON 또는 운영 오케스트레이션이
  보장해야 한다.
