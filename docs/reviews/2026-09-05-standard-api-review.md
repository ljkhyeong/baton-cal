...
완료 여부를 읽기 전에 시즌 투영 잠금을 잡는다. 완료 후 재시도는 저장된 매니페스트만 비교하므로
이 잠금이 필요 없다. 같은 시즌의 일정·이름 수신이 잠금을 보유하면 불필요하게 기다리거나
`503 SERVICE_BUSY`가 될 수 있다. 소스 경로로 확인한 결과이며 이번에 잠금 대기를 재현하지는 않았다.

**변경안:** `seasonLockRepository.acquire(seasonId)`를 완료 분기와 `requireRecoveryMode()` 뒤,
현재 시즌 상태를 대조하기 직전으로 옮긴다. 복구 ID 잠금, 완료 후 요청 내용 비교,
진행 중인 검증의 시즌 잠금은 유지한다. 완료 재시도의 잠금용 INSERT·SELECT 2회를 줄일 수 있다.

아직 구현하지 않았다. 수정 시 다른 트랜잭션이 시즌 잠금을 보유한 상태에서 완료 재시도가 `200`,
다른 내용은 `409`인지 확인하고, 진행 중인 검증은 계속 시즌 잠금을 기다리는지 검증한다.
앞선 복구 조회 정리와 함께 확인했어야 할 항목이다. 제품 코드와 테스트는 이번 검토에서 변경하지 않았다.
