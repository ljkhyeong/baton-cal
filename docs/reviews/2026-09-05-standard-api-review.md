...
다음 2건은 `f0dbf7e` 이후 구현·검증을 마쳤다.
둘 다 [RecoveryManifestService](../../src/main/kotlin/io/baton/cal/recovery/RecoveryManifestService.kt)에 있다.

| 위치 | 변경 전 | 반영한 처리 |
| --- | --- | --- |
| `verifySeason()` | 기존 시즌 매니페스트를 항상 조회하지만, 완료 기록이 있을 때만 사용했다. | `findSeasonManifest()`를 완료 기록이 있는 분기 안으로 옮겼다. 진행 중인 시즌 검증에서 SELECT 1회를 줄였다. |
| `getStatus()` | 완료 기록을 읽은 뒤에도 시즌 매니페스트 수를 COUNT했다. | 완료된 실행은 `completion.seasonCount`를 사용하고 진행 중일 때만 COUNT한다. 완료 상태 조회에서 SELECT 1회를 줄였다. |
...
`f0dbf7e`에 구현·테스트를 추가한 미커밋 상태에서
`./gradlew --no-daemon test --tests 'io.baton.cal.web.RecoveryManifestHttpTest'`를 실행했다.
Java 25.0.3·PostgreSQL 18.6 Testcontainers 환경에서 11개를 새로 실행해 모두 통과했다.
기존 상태 조회 테스트에 완료 후 매니페스트의 같은 요청 재시도·다른 요청 충돌을 보완했다.
진행·완료·빈 데이터 상태, 매니페스트 갱신, 잠금과 동시 완료 검증도 통과했다.
API 계약·설정·의존성은 유지했고 전체 테스트와 계약 ZIP 검증은 반복하지 않았다.
SQL 문자열이나 호출 횟수만 검사하는 테스트는 추가하지 않았다.
추가로 살핀 단독 함수 선언은 Spring·Jackson 진입점이어서 미사용 코드로 분류하지 않았다.

## 복구 재시도 잠금 검토 — `70828ab` 기준

[RecoveryManifestService.verifySeason()](../../src/main/kotlin/io/baton/cal/recovery/RecoveryManifestService.kt)는
완료 여부를 읽기 전에 시즌 투영 잠금을 잡고 있었다. 완료 후 재시도는 저장된 매니페스트만 비교하므로
이 잠금이 필요 없다. 같은 시즌의 일정·이름 수신이 잠금을 보유하면 불필요하게 기다리는 경로였다.

**반영:** `seasonLockRepository.acquire(seasonId)`를 완료 분기와 `requireRecoveryMode()` 뒤,
현재 시즌 상태를 대조하기 직전으로 옮겼다. 복구 ID 잠금, 완료 후 요청 내용 비교,
진행 중인 검증의 시즌 잠금은 유지한다. 완료 재시도의 잠금용 INSERT·SELECT 2회를 제거했다.

2026-09-06, `3514a97`에 구현·테스트를 추가한 미커밋 상태에서
`./gradlew --no-daemon test --tests 'io.baton.cal.web.RecoveryManifestHttpTest'`를 실행했다.
Java 25.0.3·PostgreSQL 18.6 Testcontainers 환경에서 13개를 새로 실행해 모두 통과했다.
추가한 완료 전·후 회귀 검증은 다른 트랜잭션이 실제 시즌 잠금을 보유한 채 요청을 처리한다.
완료 후 같은 요청은 `200`, 다른 내용은 `409`였고, 진행 중인 검증은 대기 시간 초과 후
`503 SERVICE_BUSY`와 `Retry-After: 1`을 반환했다. 요청 생성 코드는 기존 테스트와 함께 사용한다.
API 계약·스키마·의존성은 유지했고 전체 테스트와 계약 ZIP 검증은 반복하지 않았다.
