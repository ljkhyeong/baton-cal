...
| 변경 의미 | JSON 구조 제한, `prod` 데이터베이스 기본값 차단, 표현 변경 검증 값 전진, 재활성화·500 비노출, 데이터베이스 제한 시간의 `503 SERVICE_BUSY` 회귀 검증 |
| 상태 | 로컬 76개 기본 테스트·실행 JAR·계약 ZIP과 별도 투영 부하 테스트 검증 완료, 미게시, BATON 미고정 |
