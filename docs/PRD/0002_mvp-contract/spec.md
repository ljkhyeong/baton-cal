...
복구 완료 시각을 알 수 없으므로 이 오류에는 `Retry-After`를 넣지 않는다. 생성·회전 호출자는
복구 모드 해제를 확인한 뒤 명시적으로 다시 요청하며, 응답 유실 시 자동 재시도하지 않는 기존 계약을
유지한다. 설정은 프로세스 시작 때 읽는다. BATON 최신 전체 스냅샷·필요한 취소·시즌 이름의 재전달 완료를
확인한 뒤 모든 인스턴스에 `false`를 적용해 재시작하고, 구독 세대는 새 값을 그대로 유지한다.

CAL은 재전달 완료를 스스로 판단하거나 복구 모드를 자동 해제하지 않는다. BATON 전체 시즌의
매니페스트·완료 신호 계약은 아직 없으며, 운영자가 복구 모드 설정과 해제 순서를 보장해야 한다.
저장소 복원 훈련은 대표 계약 픽스처로 이 절차를 검증하지만 전체 원본 최신성이나 운영 복구 완료를
판정하지 않는다.
...
14. GitHub Actions가 `main` 푸시와 풀 리퀘스트에서 Java 25로 테스트와 OCI 이미지를 만들고,
    `prod` 프로필·PostgreSQL·Flyway V1~V7·DB 포함 준비 상태·비루트 실행을 실제 컨테이너로 검증한다.
...
차단하며, 전체 최신 스냅샷과 필요한 취소의 재전달 완료 확인 뒤 운영자가 해제한다.
...
테스트와 OCI 이미지를 만들고 실제 `prod` 컨테이너의 PostgreSQL 연결, Flyway V1~V7, 준비 상태,
...
- 실제 BATON 전체 시즌의 매니페스트·재전달 완료 신호와 복구 모드 해제 판단 계약. 운영 RTO/RPO,
  백업 저장소와 암호화, 비밀 관리 시스템을 포함한 실제 배포 환경 복원 훈련.
