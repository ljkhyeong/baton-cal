...
| `404` | `RESOURCE_NOT_FOUND` | 일정 항목·구독이 없거나 회전할 구독이 폐기됨 |
...
보류한 요청 제한을 도입하면 `429`도 재시도 가능한 상태에 추가한다.
...
- 현재 BATON 생산자 안정 기준은 `contracts-v1.0.0`이다. 게시·생산자 검증을 완료한
  ...
  추가한다. `rc.2`는 미게시·BATON 미고정 상태다. 새 경로를 활성화하기 전에 BATON의 사전 ID 저장,
...
- Prometheus 관측 백엔드 연결과 `prod` 관리 포트의 내부 네트워크 제한 검증.
