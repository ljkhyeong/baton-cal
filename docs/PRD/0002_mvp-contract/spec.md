...
일정 수신, 일정·구독 상태 조회, 투영 재구축, 구독 폐기, 공개 피드 조회와 readiness는 복구 모드로
차단하지 않는다.
...
유지한다. 설정은 프로세스 시작 때 읽는다. BATON 최신 전체 스냅샷과 필요한 취소의 재전달 완료를
...
14. GitHub Actions가 `main` 푸시와 풀 리퀘스트에서 Java 25로 테스트와 OCI 이미지를 만들고,
    `prod` 프로필·PostgreSQL·Flyway V1~V6·DB 포함 준비 상태·비루트 실행을 실제 컨테이너로 검증한다.
...
16. 일정 수신 결과, 일정·구독 상태 조회, 구독 생성·회전, 투영 재구축과 공통 오류의 실제 MockMvc
...
- 현재 BATON 생산자 기준은 불변 안정 릴리스 `contracts-v1.0.0`이다. `1.1.0-rc.1`은 기존
  ...
  회귀 검증, 복구 모드의 발급 차단과 내부 상태 조회도 포함한 다음 검토 후보이며, 아직 게시하거나
  BATON 생산자에 고정하지 않았다.
