...
- CAL은 BATON이 보낸 완전한 항목 스냅샷을 저장하고 iCalendar로 투영할 뿐 값을 다시
  계산하지 않는다.
- 모든 내부 경로는 BATON 서비스용 인증 주체만 호출할 수 있다. 최종 사용자 세션, 작업공간 키,
...
- `DTSTAMP`와 `LAST-MODIFIED` = `sourceUpdatedAt`을 UTC 기본 형식 날짜-시간으로 표현한 값이다.
...
- `calendar_subscription`은 현재 `token_hash` 하나만 가진다. 회전은 현재 해시를 조건으로
  한 비교 후 설정으로 새 다이제스트를 교체하며 과거 다이제스트 행을 보존하지 않는다.
...
| `409` | `SUBSCRIPTION_CONFLICT` | 회전/폐기 비교 후 설정의 동시성 충돌 |
...
- 현재 토큰 해시의 비교 후 설정 교체는 응답 전 한 트랜잭션으로 커밋한다.
...
6. 결정적인 시간대/이벤트/속성 순서, 바이트가 안정적인 재구축, ETag와 Last-Modified.
