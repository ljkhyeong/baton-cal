...
복사가 실패하면 주소 입력란을 선택해 직접 복사한다. API 키나 추가 서비스 가입은 필요하지 않다.
...
자동으로 갱신해 현재 검색·숨김 조건을 적용한다. 기존 팀·시즌 주소의
**내 캘린더 구독 해제**도 사용할 수 있다.
목록에는 구독 주소가 없으며 새 주소 발급은 접근 가능한 팀 화면에서 수행한다.
계정 비활성화·팀 권한 회수·구성원 활동 중지 시 구독 해제를 자동 요청한다. 해제 전까지 기존 주소를
사용할 수 있다. 해제 후에도 앱에 저장된 일정은 직접 삭제한다.

여러 구독을 정리하려면 **여러 구독 선택**을 누르고 해제할 항목을 고른다. 불러온 구독 중
