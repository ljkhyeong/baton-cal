...
BATON에서 주소를 발급하면 등록 안내가 바로 펼쳐진다. **사용할 캘린더**를 선택하고 **구독 주소 복사**를
...
나머지 요청을 보내지 않는다. **해제 작업 중단**을 누르거나 화면을 떠나도 이미 보낸 요청은
...
중단하며 배경 탭에서는 자동 조회하지 않는다. 90초가 지나면 **자동 확인 종료** 안내를 표시하고,
조회에 실패하면 중단 사유를 표시한다. **상태 다시 확인**을 누르면 종료 안내가 사라지고 다시 시작한다.
