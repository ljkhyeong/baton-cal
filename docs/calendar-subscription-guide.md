...
등록한다. 파일을 가져오면 이후 일정 변경·취소는 자동 반영되지 않는다.
구독은 읽기 전용이며 일정 변경과 참여 권한은 BATON에서 관리한다.

BATON에서 주소를 발급하면 등록 안내가 바로 펼쳐진다. **캘린더 앱**을 선택하고 **구독 주소 복사**를
누른 뒤 선택한 앱에 붙여 넣는다. Google은 **Google 등록 화면 열기**, Outlook은 개인·회사/학교 계정별
바로가기를 제공하고, Apple은 공식 등록 안내를 연다. 링크에는 구독 주소가 포함되지 않으며 새 탭에서 열린다.
복사가 실패하면 주소 입력란을 선택해 직접 복사한다. 별도 연동 서비스 가입은 필요하지 않다.

### 구독 조회·해제

BATON의 열람자도 열린 시즌의 주소를 발급할 수 있다. **내 계정 → 내 캘린더 구독**에서 팀·시즌별
본인 구독 기록을 모아 보고, 항목을 펼쳐 최신 상태를 확인하거나 해제한다. 팀 접근 권한을 잃은
구독과 해제한 구독도 표시한다. 팀·시즌 이름으로 전체 구독을 검색하고 **해제된 구독 숨기기**로
목록을 정리할 수 있다. 해제 처리 중인 구독은 계속 표시한다. 해제 완료를 확인하면 목록도
자동으로 갱신해 현재 검색·숨김 조건을 적용한다. 기존 팀·시즌 화면의
**내 캘린더 구독 해제**도 사용할 수 있다.
목록에는 구독 주소를 표시하지 않는다. 새 주소는 접근 권한이 있는 팀 화면에서 발급한다.
계정 비활성화·팀 권한 회수·구성원 활동 중지 시 구독 해제를 자동 요청한다. 해제 전까지 기존 주소를
사용할 수 있다. 해제 후에도 앱에 저장된 일정은 직접 삭제한다.

### 여러 구독 해제

**여러 구독 선택**을 누르고 해제할 항목을 고른다. 불러온 구독 중
최대 20개까지 선택하며 해제된 구독은 제외한다. **불러온 항목 모두 선택**은 아직 불러오지 않은
구독까지 선택하지 않는다. 더 보기로 불러온 항목이 20개를 넘으면 **처음 20개 선택**을 제공한다.
검색·필터를 바꾸거나 **선택 초기화**를 누르면 체크 표시가 지워진다. **선택한 구독 N개 해제**를 누른 뒤 팀·시즌 목록을
확인하고 **N개 구독 해제**를 누른다. 결과는 항목별로 **해제됨**, **해제 처리 중**,
**해제 여부 확인 필요**로 표시한다.
실행 전 다른 구독으로 바뀐 항목은 건너뛰고 확인이 필요하다고 안내한다. 계정 확인이 실패하면
나머지 요청을 보내지 않는다. **남은 구독 해제 중단**을 누르거나 화면을 떠나도 이미 보낸 요청은
처리될 수 있다. 미완료 항목은 **선택 마치기**를 누른 뒤 항목을 펼쳐 상태를 다시 확인한다.

### 구독 상태 확인

확인한 상태와 시각은 항목을 접어도 현재 계정 화면에 남는다. 표시 시각은 BATON에서 구독 상태를
확인한 시간이다. 캘린더 앱의 갱신 시간은 아니다. 처리 중인 항목을 열어 두면
3초 간격으로 최대 90초간 상태를 자동 확인한다. 완료하거나 조회에 실패하거나 항목을 닫으면
중단하며 배경 탭에서는 자동 조회하지 않는다. 90초가 지나면 자동 확인을 멈추고 **상태 다시 확인**을
안내한다. 조회에 실패하면 중단 사유를 표시한다. **상태 다시 확인**을 누르면 안내가 사라지고 다시 시작한다.
자동 확인은 새 구독 주소를 발급하거나 해제 요청을 반복하지 않는다.

**주소 사용 가능**은 구독 주소가 유효하다는 뜻이다. 캘린더 앱에 등록했는지는 확인하지 않는다.

### 구독 주소 관리

구독 주소를 아는 사람은 일정을 볼 수 있으므로 게시판·메신저·오류 제보에 붙이지 않는다.
...
| 앱 | 등록 방법 | 갱신 안내 |
| --- | --- | --- |
| Google Calendar 웹 | 컴퓨터에서 다른 캘린더 옆 `+` → URL로 추가 → 구독 주소 입력 | 변경은 Google의 갱신 주기에 따라 반영된다. 모바일 앱에서는 등록할 수 없으므로 PC 웹에서 등록한다. |
...
제공한다. 계정 구독 목록을 포함한 로컬 화면·서버 검증과 브랜치 통합 상태는 [인수인계](../HANDOFF.md)를
따른다. 실제 외부 캘린더 앱 검증은 별도다.
