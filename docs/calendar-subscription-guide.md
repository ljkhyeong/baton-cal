...
목록에는 구독 주소가 없으며 새 주소 발급은 접근 가능한 팀 화면에서 수행한다. 권한 회수와 활동 중지는 자동
폐기를 요청하며, CAL에 전달되기 전에는 기존 주소가 작동할 수 있다. 이미 내려받은 일정은 앱에서 제거한다.
...
검색·필터를 바꾸면 이전 선택은 지워진다. **선택한 구독 N개 해제**를 누른 뒤 팀·시즌 목록을
확인하고 실행한다. 결과는 항목별로 **해제됨**, **해제 처리 중**, **결과 확인 필요**로 표시한다.
실행 전 다른 구독으로 바뀐 항목은 건너뛰고 확인이 필요하다고 안내한다. 계정 확인이 실패하면
나머지 요청을 보내지 않는다. **남은 요청 중단**을 누르거나 화면을 떠나도 이미 보낸 요청은
처리될 수 있다. 미완료 항목은 **선택 마치기**를 누른 뒤 항목을 펼쳐 상태를 다시 확인한다.

확인한 상태와 시각은 항목을 접어도 현재 계정 화면에 남는다. 표시한 시각은 BATON 구독 상태를
확인한 시간이며 Google·Apple·Outlook이 일정을 가져간 시간이 아니다. 처리 중인 항목을 열어 두면
