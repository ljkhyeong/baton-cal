...
- 확인일: 2026-09-12
- 기준: CAL `79520a4`, BATON 원격 main `c94469e`의 API·웹훅 연결과 실행 조건 재검토
...
| 내부 API HTTPS | Spring Boot PEM SSL bundle | 인증서 파서·HTTPS 서버·파일 갱신 감시 코드 | 선택형 `tls` 프로필, 인증서 자동 반영과 실제 HTTPS 검증 추가 |
...
없으며 운영 연동은 기존 도구의 설정으로 처리한다. 다만 BATON은 내부 API에도 HTTPS를 요구하는데
...
인증서 파일 갱신도 SSL bundle의 기본 감시 기능으로 연결했다. CAL 재시작 없이 새 HTTPS 연결에
반영하며 발급·갱신 도구와 실제 Secret 마운트는 운영에서 연결한다. [인증서 갱신 반영](integration-runtime.md#인증서-갱신-반영)
