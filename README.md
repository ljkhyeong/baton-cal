...
5. 현재 세대 자격 증명을 아직 발급하지 않은 상태에서 BATON이 모든 시즌의 최신 전체 스냅샷을
   다시 전달한다. 복원 DB에만 활성 상태로 남을 항목의 `CANCELLED` 스냅샷과 사용 중인 시즌의 최신
   표시 이름도 포함하고 완료를 확인한다.
6. 최신 상태 복구가 끝난 뒤 모든 인스턴스에 `BATON_CAL_RECOVERY_MODE=false`를 적용해 재시작한다.
...
다만 CAL은 BATON 전체 재전달 완료를 알려 주는 매니페스트나 완료 신호를 아직 받지 않는다.
운영자 또는 상위 오케스트레이션이 복구 모드를 켜고, 완료를 확인한 뒤 해제해야 한다.
...
중복·낮은 개정 번호는 현재 값을 반환한다. 복구 모드에서도 사용할 수 있다. BATON의 이름 전달 연결과
실제 캘린더 앱의 이름 갱신 검증은 별도 작업이다. BATON HTTP 클라이언트와 CAL 후보 컨테이너의
이름 전달·피드 반영은 검증했지만, 원본 변경 캡처·아웃박스·복원 재전달은 아직 연결하지 않았다.
...
스모크는 이미지의 Java 25와 비루트 실행, Flyway V1~V7 적용, 분리된 관리 포트의 DB 포함 준비 상태와
