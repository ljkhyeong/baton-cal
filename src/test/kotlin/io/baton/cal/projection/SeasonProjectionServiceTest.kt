...
class SeasonProjectionServiceTest {
    ...
    fun `투영이 없으면 관측 시각의 초를 사용한다`() {
        ...
        doReturn(null).`when`(projectionRepository).findBySeasonId(SEASON_ID)
    ...
    fun `같은 초이거나 시계가 뒤로 가면 이전 값에서 정확히 1초 전진한다`() {
        val current = projection(lastModified = Instant.parse("2026-08-14T03:04:05Z"))
        doReturn(current).`when`(projectionRepository).findBySeasonId(SEASON_ID)
        ...
        val exactlyOneSecondLater = current.lastModified.plusSeconds(1)
    ...
    private fun projection(lastModified: Instant): SeasonFeedProjectionRow = SeasonFeedProjectionRow(
        seasonId = SEASON_ID,
