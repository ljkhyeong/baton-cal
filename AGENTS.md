...
- 컬렉션, 널 처리, 문자열·바이트 인코딩, Base64, 16진수, 파일 편의와 자원 수명에는 Kotlin 표준
  라이브러리를 먼저 쓴다. 시간, 암호화, URI, 네트워크와 UUID처럼 JVM 고유 기능은 JDK API를 사용한다.
- 직접 파서·직렬화기·경로 판별·재시도·충돌 검사를 만들기 전에 Spring과 고정한 라이브러리 API가
  계약을 정확히 만족하는지 먼저 확인하고 그 API를 사용한다.
...
- 종류는 `기능`, `수정`, `문서`, `테스트`, `설정`, `빌드`, `배포`, `운영`, `리팩터` 중에서 고른다.
